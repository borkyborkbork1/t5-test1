...
namespace TiltFive
{
    ...
    public static class Input
    {
        ...
        private static Dictionary<ControllerIndex, WandControlsState?> currentWandStates;
        private static Dictionary<ControllerIndex, WandControlsState?> previousWandStates;
        ...
        public static bool GetButton(WandButton button, ControllerIndex controllerIndex = ControllerIndex.Primary)
        {
            ...
            return wandState?.GetButtonState(button) ?? false;
        }
        ...
        {
            var wandState = currentWandStates[controllerIndex];
            pressed = wandState?.GetButtonState(button) ?? false;
        ...
        public static bool GetButtonDown(WandButton button, ControllerIndex controllerIndex = ControllerIndex.Primary)
        {
            ...
            var pressed = wandState?.GetButtonState(button) ?? false;
            ...
            var previouslyPressed = previousWandState?.GetButtonState(button) ?? false;
        ...
        /// <returns>Returns a float representing how much the trigger has depressed by the user, from 0.0 (released) to 1.0 (fully depressed).</returns>
        ...
        /// </summary>
        /// <param name="triggerDisplacement">A float representing how much the trigger has depressed by the user, from 0.0 (released) to 1.0 (fully depressed).</param>
        /// <param name="controllerIndex">Unless specified, the state of the dominant-hand wand trigger is used.</param>
        ...
        internal static bool TryGetWandControlsState(out WandControlsState? wandControlsState, ControllerIndex controllerIndex = ControllerIndex.Primary)
        {
            int result = 1;
        ...
        static Input()
        {
            ...
            currentWandStates = new Dictionary<ControllerIndex, WandControlsState?>() {
                { ControllerIndex.Primary, null },
                { ControllerIndex.Secondary, null }
            };

            previousWandStates = new Dictionary<ControllerIndex, WandControlsState?>(){
                { ControllerIndex.Primary, null },
                { ControllerIndex.Secondary, null }
            };
